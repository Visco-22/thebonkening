...
After a random number of commands (5–12 by default), prints bonk, waits 5 seconds, and closes the terminal.
...
MAX – Randomly chosen limit of commands (5–12) before the shell prints bonk and closes.
...
If CMD >= MAX, prints bonk, waits 5 seconds, and terminates the shell.
DEBUG trap
...
The script appends a check to ~/.bashrc: [[ -f "$HOME/.thebonkening_active" && -z "$THEBONKENING_ACTIVE" ]] && source "$HOME/.thebonkening.sh"
...
Because the script closes your shell, test it in a subshell or terminal multiplexer (tmux/screen) first.
You can temporarily increase MAX to avoid closing too early during testing: MAX=50
...
Command limit: Change MAX=$((RANDOM%8+5)) to set the min/max number of commands before the terminal closes.
Bonk delay: Adjust the sleep 5 in limit() to change how long the terminal waits before closing.
